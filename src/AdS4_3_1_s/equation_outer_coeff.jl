...
ABCS[2] = 4 * u ^ 3 * x0 * x1
ABCS[3] = 0
ABCS[4] = 4 * Sh ^ 2 * x3 + x0 * (-8 * St * x5 + x2 * (Bd * Bp * x3 ^ 2 + Gd * Gp) + x6 * (x3 * (Gh * St + Gt * Sh) + x4 * (-Fxh * Sp - Fyt * Sp + 2 * Sc - x12 * xi_xy + x17 * x18 + x18 * x19 + x18 * x20)) + x7 * (-8 * Sd * Sp + x3 * (-Bh * x15 + 2 * Bt * Gh - Fxh * x10 - Fyt * x10 + 4 * Gc - 4 * Gp * xi_xy + x17 * x21 + x19 * x21 + x20 * x21) - x4 * (2 * Fxp * Fyp - Gt * x9))) + x6 * (-Gh * x5 + Sp * x3 * xi_yy + x3 * (Bh * Sh + Fyh * Sp - Ss)) + x7 * (x3 * (-2 * Bh ^ 2 + 2 * Bs - Fyh * x8 + Fyp ^ 2 - 2 * Gh ^ 2) + x4 * (Bh * x9 + Fyh * x10 - 2 * Gs) + xi_yy * (2 * Gp * x4 - x3 * x8)) + (-x13 * x4 * (-Fxt * x11 + Gb * S + x15 * (St + x14)) + x13 * xi_xx * (x11 * x4 + x3 * (Bp * S + x12)) + x3 * (-Bb * x16 + 2 * Bp * Fxt * x7 - Bt ^ 2 * x16 + Fxp ^ 2 * x7 + 4 * Fxt * S * Sp - Gt ^ 2 * x16 - Sb * x6 + 4 * St ^ 2 - 4 * St * x14)) * exp(2 * B)
    nothing
end
